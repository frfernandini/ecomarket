package com.ecomarket.msvc_proveedores.exceptions;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorDTO> handleProveedorException(ProveedorException exception){
        if(exception.getMessage().contains("no encontrado")){
            Map<String,String> errorMap = Collections.singletonMap("Proveedor no encontrado",exception.getMessage());
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(this.createErrorDTO(HttpStatus.NOT_FOUND.value(), new Date(),errorMap));
        }else{
            Map<String,String> errorMap = Collections.singletonMap("Proveedor existente",exception.getMessage());
